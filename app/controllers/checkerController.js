...
// fungsi untuk mengecek apakah ada task mengirim email atau tidak
const checker = async () => {
    const date = new Date()
    ...
    // mengecek kedalam database apakah pada rentang waktu jam,menit dan tanggal saat ini ada task email yang harus dikirim
    // 'scheduleTime.minutes': { $lte: String(parseInt(minutes)+1), $gte: String(parseInt(minutes)-1) } => ini mengecek task pada rentang waktu antara 1 menit sebelum atau 1 menit setelah, ini dilakukan untuk mencegah task yang terlewat karena terhambat mengerjakan task sebelumnya (bisa jadi saat mengirim email waktunya lama sekali)
    ...
    const getData = await Model.find({ platform: 'email', 'scheduleTime.hours': String(parseInt(hours)), 'scheduleTime.minutes': { $lte: String(parseInt(minutes)+1), $gte: String(parseInt(minutes)-1) }, lastSent: { $ne: `${day}-${month}-${year}` }, status: 'active', softDelete: null }).sort({ receiversCount: 'asc' })
    if(getData.length <= 0) {
