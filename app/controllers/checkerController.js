...
const checker = async () => {
    ...
    const getData = await Model.find({ platform: 'email', 'scheduleTime.hours': parseInt(hours), 'scheduleTime.minutes': { $lte: parseInt(minutes)+1, $gte: parseInt(minutes) }, lastSent: { $ne: `${day}-${month}-${year}` }, status: 'active', softDelete: null }).sort({ receiversCount: 'asc' })
    if(getData.length <= 0) {
